/*
In the text box below, provide January 31's rolling 3 day average of total transaction amount processed per day
and provide the link to your solution.
*/
...
- "rolling 3 day average" means the average of the current date and the previous two dates
- "total transaction amount processed per day" means the SUM(transaction_amount) grouped for each date
- We can disregard any potential time zone issues and take the time stamps at face value
- Using PostgreSQL syntax here
*/

-- Aggregate total transaction amounts per day
...
-- Calculate the rolling average as defined in the assumptions
rolling_avg AS (
	SELECT
  		transaction_date,
...
-- Finally, SELECT the average for the 2021-01-31 date
